...
- **人工手動自訂優先 (Manual Overrides First)**：
  - 系統支援 `manual_overrides.json`。只要該檔案中指定了特定原則或卡牌條目，權重永遠高於 `translation_db.json` 與 Gemini API，且腳本執行時嚴禁覆蓋此檔案。
- **快取優先原則 (Cache First)**：
  - 翻譯資料一律先比對 `manual_overrides.json` 與 `translation_db.json`。
  - 只有在偵測到投影片新增原則或修改英文原文且未在覆蓋庫中時，才允許呼叫 Gemini API (`gemini-3.8-flash`)。
...
- **金句 (`☆` 項目) 處理**：
  - 簡報中以 `☆` 包裹的篇章引導金句（共 53 則）不列入 458 條簡報卡牌編號，而是作為「六大篇章的核心精神指引金句」，展示於網頁「核心原則」各篇章頂部橫幅（Chapter Banner）、Markdown 手冊各篇開頭以及出版書籍章節頁中。

...
4. **關係圖譜 🕸️ (Connection Graph)**：
   ...
   - **拓撲結構**：
     ...
     - 篇章結構虛線 + 簡報卡牌交集重疊實線（線寬依共用數加粗並附計數標籤）+ 跨章節哲學聯動線（Semantic Synergies）。
   - **互動控制**：支援畫布平移、以游標為中心的平滑縮放（0.35x~2.5x）、節點任意拖曳重組、懸停高亮關聯路徑、篇章篩選、視角重置，以及「暫停/繼續模擬」按鈕（鎖定版面與節省 CPU）。
   - **滑出式詳細抽屜 (Sliding Drawer)**：點擊任一原則即滑出右側抽屜，呈現三語文本、關聯原則跳轉膠囊與對照卡牌一覽（點擊卡牌直達簡報卡牌檢視）。

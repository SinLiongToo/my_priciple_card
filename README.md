...
2. **四種檢視視角**：
   ...
   - **關係圖譜 🕸️ (Connection Graph)**：原生 HTML5 Canvas 物理力導向知識圖譜。動態呈現 6 大篇章樞紐、20 大核心原則與簡報卡牌之關聯網絡，支援視角縮放平移、節點拖曳互動、篇章過濾，以及滑出式詳細資訊抽屜（含三語文本、關聯原則跳轉與卡牌直達）。
...
### 2026-09-13：升級「關係圖譜 🕸️」—— 全面延伸連結 458 張簡報卡牌 (三層全景星系)
- **🪐 原則衛星動態展開與全景星系雙模式**：
  - **視角切換開關**：提供「`🌐 原則骨幹 (20)`」與「`✨ 全景星系 (458卡牌)`」一鍵平滑切換。
  - **衛星群動態展開 (Satellite Mode)**：支援直接在畫布上雙擊原則節點，或於滑出抽屜點擊「🪐 展開/收合卡牌」，以所屬原則為核心向外炸開環形衛星卡牌節點。
  - **跨原則橋接節點 (Bridge Cards)**：被多條原則共用的卡牌以專屬金色標記（`#fbbf24`），並在多原則間形成實質交叉連結橋樑，視覺化呈現心法跨域互通。
  - **卡牌節點互動與預覽**：懸停卡牌小節點即時顯示浮動摘要 Tooltip（含卡牌編號與中文精要），點擊卡牌即滑出完整三語抽屜並支援一鍵直達簡報卡牌檢視。
  - **Canvas 60 FPS 物理效能優化**：導入空間距離剪枝（Spatial Distance Pruning）與動態阻尼，即使全開 484 個節點亦流暢平滑、無卡頓延遲。

### 2026-09-13：新增第 4 標籤「關係圖譜 🕸️ (Connection Graph)」
- **🕸️ 全新互動式知識關係圖譜（第 4 個 Tab）**：
  - 採用原生 HTML5 Canvas 物理力導向模擬引擎（Force-directed Simulation），完全零外部重型函式庫依賴，兼具極致流暢度與離線運行能力。
  - **知識網絡拓撲**：
    - 6 大篇章樞紐節點（Hub Nodes）以六色主題呈現。
    - 20 大核心原則節點（Principle Nodes）自動聚類於所屬篇章，節點大小依對照卡牌數量動態縮放。
    - 連線機制完整呈現「篇章結構連線」、「卡牌交集重疊關聯線（線寬依共同對照卡牌數加乘）」以及「跨章節核心心法聯動線（Semantic Synergies）」。
  - **互動與操作體驗**：
    - 支援畫布平移（Pan）、滾輪與雙指平滑縮放（Zoom）、節點自由拖曳重組。
    - 懸停節點即時高亮相關聯路徑與節點、淡化其餘元素。
    - 點擊原則節點自動滑出右側抽屜（Detail Drawer），提供完整三語對照內容、關聯原則跳轉膠囊以及對照簡報卡牌一覽（點擊卡牌直達簡報卡牌檢視）。
    - 提供篇章獨立篩選、重置視角與暫停/繼續物理模擬控制項。
